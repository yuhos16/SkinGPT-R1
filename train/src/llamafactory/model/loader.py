...
def load_model(
    ...
    print(f"🚀 [SkinGPT-Model] Start loading model from: {model_args.model_name_or_path}")
    ...
    if model is None and not lazy_load:
        ...
        if getattr(config, "model_type", None) == "qwen2_5_vl":
            from llamafactory.model.skin_vlm_adapter import SkinVLModelWithAdapter
            print("[✓] Detected model_type == 'qwen2_5_vl', injecting SkinVLModelWithAdapter")
            ...
            if finetuning_args.finetuning_type == "freeze":
                print("🔒 [SkinGPT-Model] Applying Freeze Strategy: freezing backbone, training MoE Adapter & Distill Head...")
                model.freeze_all_but_distill()  
                with torch.no_grad():
                    ref = next(model.parameters())
                    model.logit_bias_scale.data = torch.tensor(1.5, device=ref.device, dtype=ref.dtype)  
                print(f"   -> [DBG] logit_bias_scale initialized to: {float(model.logit_bias_scale.detach().float().cpu())}")

            trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
            total     = sum(p.numel() for p in model.parameters())
            print(f"📊 [SkinGPT-Model] Params Stats: Trainable {trainable:,} / Total {total:,} ({100 * trainable / total:.4f}%)")

            return model

        if model_args.mixture_of_depths == "load":
            # load_class.register(type(config), SkinVLModelWithAdapter, exist_ok=True)
    ...
    print("="*60)
    print(f"✅ [SkinGPT-Model] Model Loaded Successfully!")
    print(f"   -> {param_stats}")
    print("="*60)
