...
class AlpacaDatasetConverter(DatasetConverter):
    def __call__(self, example: dict[str, Any]) -> dict[str, Any]:
        ...
        output = {
            ...
            "skin_labels": example.get("skin_labels", None),
        }
        return output
...
def align_dataset(
    ...
    if "skin_labels" in column_names and hasattr(dataset, "add_column"):
        try:
            print(f"\n🔍 [Align-Fix] Detected 'skin_labels' in raw data. Re-stitching to aligned dataset...")
            
            labels_to_add = dataset["skin_labels"]
            
            aligned_dataset = aligned_dataset.add_column("skin_labels", labels_to_add)
            print(f"✅ [Align-Fix] Successfully preserved 'skin_labels'.")
        except Exception as e:
            print(f"⚠️ [Align-Fix] Failed to stitch skin_labels: {e}")
