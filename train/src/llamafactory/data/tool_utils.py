...
GLM4_TOOL_PROMPT = (
    "You are ChatGLM, an AI assistant based on the GLM-4 language model trained by Zhipu AI. "
    "Provide appropriate answers and support for the user requests.\n\n# Available tools{tool_text}"
)
...
class GLM4ToolUtils(ToolUtils):
    ...
    def tool_formatter(tools: list[dict[str, Any]]) -> str:
        ...
        for tool in tools:
            tool = tool.get("function", "") if tool.get("type") == "function" else tool
            tool_text += "\n\n## {name}\n\n{body}\nUse JSON to specify the arguments when calling these functions.".format(
