...
class MultiModalDataCollatorForSeq2Seq(DataCollatorForSeq2Seq):
    ...
    def __call__(self, features: list[dict[str, Any]]) -> dict[str, "torch.Tensor"]:
        ...
        # --- SkinGPT-R1 Mod: Extract custom features safely ---
        # Cache specifically for teacher_feat and skin_labels
        # Using a dictionary mapping index to value to handle potential missing keys gracefully
        teacher_feats_cache = {}
        skin_labels_cache = {}

        for i, f in enumerate(features):
            # 1. Extract teacher_feat
            if "teacher_feat" in f:
                teacher_feats_cache[i] = f.pop("teacher_feat")
            
            # 2. Extract skin_labels
            if "skin_labels" in f:
                skin_labels_cache[i] = f.pop("skin_labels")
        # --- End Mod ---

        from PIL import Image

        for f in features:
            img_paths = f.pop("images", None) or f.pop("_images", None) or []
            loaded_images = []
            ...
            for p in img_paths:
                try:
                    ...
                    if isinstance(p, str) and os.path.exists(p):
                        ...
                        loaded_images.append(p.convert("RGB"))
                    else:
                        print(f"[!] Invalid image path or object: {p}")
                except Exception as e:
                    print(f"[✗] Failed to load image {p}: {e}")

            f["images"] = loaded_images
            f.pop("_images", None)
        ...
        # --- SkinGPT-R1 Mod: Inject back custom features ---
        bsz = features["input_ids"].size(0)
        
        # 1. Restore Teacher Feat
        if len(teacher_feats_cache) > 0:
            # Prepare a list ordered by index
            tf_list = [teacher_feats_cache[i] for i in range(bsz) if i in teacher_feats_cache]
            if len(tf_list) == bsz:
                try:
                    # Using float32 is safer for general compatibility, model can cast to bf16
                    features["teacher_feat"] = torch.stack(
                        [torch.tensor(t, dtype=torch.float32) for t in tf_list]
                    )
                except Exception as e:
                    print(f"[!] Error stacking teacher features: {e}")
            else:
                print(f"[!] Warning: Partial teacher_feat batch. {len(tf_list)}/{bsz} found.")

        # 2. Restore Skin Labels
        if len(skin_labels_cache) > 0:
            sl_list = [skin_labels_cache[i] for i in range(bsz) if i in skin_labels_cache]
            if len(sl_list) == bsz:
                try:
                    features["skin_labels"] = torch.tensor(sl_list, dtype=torch.long)
                except Exception as e:
                    print(f"[!] Error stacking skin labels: {e}")
            else:
                print(f"[!] Warning: Partial skin_labels batch. {len(sl_list)}/{bsz} found.")

        # --- Debug Logging (First Batch Only) ---
        if not getattr(self, "has_logged_first_batch", False):
            print("\n" + "="*40)
            print("🚀 [SkinGPT-Collator] First Batch Sanity Check:")
            print(f"   Batch Size      : {bsz}")
            
            if "teacher_feat" in features:
                print(f"   ✅ teacher_feat : Present | Shape {features['teacher_feat'].shape}")
            else:
                print(f"   ❌ teacher_feat : MISSING (Cache len: {len(teacher_feats_cache)})")
                
            if "skin_labels" in features:
                print(f"   ✅ skin_labels  : Present | Shape {features['skin_labels'].shape} | Vals: {features['skin_labels'].tolist()}")
            else:
                print(f"   ❌ skin_labels  : MISSING (Cache len: {len(skin_labels_cache)})")
            print("="*40 + "\n")
            self.has_logged_first_batch = True
        # --- End Mod ---
